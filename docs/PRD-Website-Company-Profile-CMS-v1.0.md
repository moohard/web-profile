...
> **Catatan dokumen:** v1.0 **memfinalkan tech stack** (Open Item #6): **Laravel + Inertia + React**. Seluruh Open Items tuntas kecuali **#3 (custom fields)** yang sengaja ditunda (Lampiran A). Ditambahkan requirement **SEO, aksesibilitas (WCAG), dan konversi gambar WebP** secara eksplisit. Spesifikasi inti final.
...
**`widget_placements`** — `id`, `widget_id` (FK), `position` (enum), `scope` (enum: ALL/ONLY/EXCEPT), `sort_order`
...
**`site_settings`** — konfigurasi global; termasuk **WhatsApp** (`whatsapp_number`, `whatsapp_enabled`, `whatsapp_message`), logo, footer, SEO default, sosmed, Maps. Entri teks yang tampil mengikuti pola terjemahan.
...
- **Autentikasi & role:** autentikasi bawaan Laravel (Fortify/Breeze) + `spatie/laravel-permission` (Admin/Editor/Author).
