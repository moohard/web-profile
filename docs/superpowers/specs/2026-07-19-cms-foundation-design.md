...
**Status:** Spec desain pondasi (revisi 1) — menunggu implementasi plan
**Revisi 1 (19 Juli 2026):** koreksi visibilitas role Editor (Tampilan → Admin-only); penegasan resolver routing catch-all; URL runtime pakai helper native Laravel; catatan verifikasi versi starter & konvensi settings hibrida.
...
- Inertia v3 + React 19 + Tailwind v4 + Radix (shadcn) + Wayfinder + `@inertiajs/vite` (enabler SSR) sudah terkonfigurasi. *(Verifikasi versi tepat & API — termasuk hook `useHttp` — langsung dari `composer.json` / `package.json` starter yang di-scaffold.)*
...
3. **`site_settings`** — **tidak** dibuat tabel custom. Non-teks pakai `spatie/laravel-settings` (kelas `SiteSettings`, `SeoSettings`, `WhatsappSettings` di `app/Settings/*`). Teks i18n pakai tabel tunggal `setting_translations` (`key`, `language_id`, `value`). **Konvensi:** buat daftar eksplisit key mana yang diterjemahkan (via `setting_translations`) vs non-teks (via kelas Settings), beserta helper pengambilan yang konsisten (mis. `setting_translated('footer_text')` vs `app(SiteSettings::class)->...`).
...
|---|---|---|---|
| Seluruh menu | ✓ | ✓ (selain Tampilan & Sistem) | terbatas |
| Konten | ✓ semua | ✓ semua | ✓ miliknya sendiri |
| Halaman + **mode code** | ✓ | ✓ (template saja) | ✗ |
| Tampilan (menu/widget/template) | ✓ | ✗ | ✗ |
| Media | ✓ | ✓ | ✓ |
...
> **Catatan implementasi (penting):** pola satu-segmen `/{type}` (arsip) dan `/{slug}` (custom page) — juga dua-segmen `/{locale}/{type}` vs `/{locale}/{slug}` — saling tumpang tindih dan **tidak bisa dibedakan router Laravel secara native**. Urutan pencocokan di atas **wajib** diwujudkan lewat **satu resolver / catch-all controller** (atau route-model-binding berbasis DB yang mengecek `content_types.slug` lalu `page_translations.slug`), bukan beberapa rute Laravel terpisah yang dicocokkan berurutan begitu saja.
...
- **Admin**: semua grup.
- **Editor**: Dashboard, Konten, Halaman, Interaksi, Media. **Tanpa Tampilan.**
- **Author**: Dashboard, Konten (milik sendiri), Media.
- Grup **Tampilan** (Menu, Widget, Template) dan item "Sistem → Pengguna, Pengaturan, Bahasa, Gaya bahasa, AI configs, Kriteria penilaian" hanya Admin.
...
- Canonical URL absolut via helper native Laravel (`url()` / `URL::to()` / `route()`) dengan `APP_URL` terkonfigurasi.
...
- URL absolut via helper native Laravel (`url()` / `route()`).
